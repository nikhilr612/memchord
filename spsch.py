...
class Scheduler:
	...
	def schedule_new_cards(self, n):
		if self.last_card + n < len(self.deck['cards']):
			self.scheduled_cards.extend(range(self.last_card+1,self.last_card+n+1));

	def get_study_info(self):
		return "Total cards: " + str(len(self.deck['cards'])) + ", For Review: " + str(len(self.scheduled_cards));
